...
async function getApiBase() {
  try {
    ...
    console.error("GitHub raw API fetch failed:", err.message);
    return null;
  }
}
...
async function convertFont(text) {
  try {
    ...
    const res = await axios.get(`${baseUrl}/api/font`, { params: { id: 16, text } });
...
module.exports.onStart = async function ({ api, args, event }) {
  ...
  const baseUrl = await getApiBase();
  if (!baseUrl) return api.sendMessage("❌ API fetch korte parlam na!", event.threadID, event.messageID);
  ...
  try {
    const res = await axios.get(`${baseUrl}/api/sakura`, { params: { query: input, userId } });
